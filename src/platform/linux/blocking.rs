...
pub fn kill_blocked_processes() {
    ...
    for (pid, process) in system.processes() {
        ...
        // Try to get full executable path for better matching
        let exe_name = process.exe()
            .and_then(|path| path.file_name())
            .and_then(|name| name.to_str())
            .unwrap_or(&process_name);

        // Clean up wrapper prefixes and suffixes
        // e.g., ".mousepad-wrapped" -> "mousepad"
        let mut clean_name = exe_name.trim_start_matches('.');

        // Remove -wrapp* suffix (handles -wrapped, -wrapp, etc.)
        if let Some(pos) = clean_name.find("-wrapp") {
            clean_name = &clean_name[..pos];
        }

        if should_block_process(clean_name) {
