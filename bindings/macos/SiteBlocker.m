        ...
        dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
          ...
          if (axError != kAXErrorSuccess || !window) {
            ...
            axError = AXUIElementCopyAttributeValues(
                appElement, kAXWindowsAttribute, 0, 1, &windowArray);

            if (axError == kAXErrorSuccess && windowArray) {
              if (CFArrayGetCount(windowArray) > 0) {
                window = (AXUIElementRef)CFRetain(
                    CFArrayGetValueAtIndex(windowArray, 0));
              }
          ...
          if (urlField) {
            ...
            if (axError == kAXErrorSuccess) {
              ...
              // Fallback approach
              NSLog(@"Failed to set URL (AX error: %d), using fallback",
                    axError);
